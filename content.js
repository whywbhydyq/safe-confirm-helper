...
${FINAL_FORMAT}`;
  ...
  function blankTask() {
    return { active: false, status: "idle", taskId: "", conversationKey: "", startedAt: 0, loopCount: 0, auditCount: 0, promptInjected: false, lastFinal: null, lastGateReason: "", stopReason: "" };
  }

  function blankSignals() {
    return { riskWord: false, staleProgress: false, finalUiSeen: false, reasons: [], lastSignalAt: 0, lastMessageHash: "" };
  }
  ...
  function cleanSettings(value) {
    ...
    const continuePrompt = String(next.continuePrompt || CONTINUE).trim();
    next.continuePrompt = !continuePrompt || legacyContinuePrompt(continuePrompt) ? CONTINUE : continuePrompt;
    return next;
  }
  ...
  async function saveSettings() {
    ...
    try { await chrome.storage.local.set({ settings }); } catch { localStorage.setItem(STORE, JSON.stringify(settings)); }
  }

  function sessionRuntime() {
    return {
      continueCount: runtime.continueCount,
      sendFailureCount: runtime.sendFailureCount,
  ...
  async function saveSession() {
    try {
      await chrome.storage.local.set({ [SESSION_STORE]: { conversationKey: getConversationKey(), task: { ...task }, runtime: sessionRuntime() } });
    } catch {}
  }
  ...
  function text(el) {
    ...
    return [...new Set([aria, value, el.textContent, el.innerText].filter(Boolean).map((x) => x.trim()).filter(Boolean))].join(" ").replace(/\s+/g, " ").trim();
  }
  ...
  function scan() {
    if (!settings.enabled) { setCandidate(null, false); return null; }
    const modalRoots = findModalRoots();
    if (modalRoots.length) return scanRoots(modalRoots, true);
    return scanRoots([document.body || document.documentElement], false);
  }

  function requestScan() {
    if (settings.autoConfirm && document.visibilityState === "hidden") { const now = Date.now(); if (now - lastHiddenScan < 250) return; lastHiddenScan = now; scan(); return; }
    if (scanTimer) return;
    scanTimer = delay(() => { scanTimer = 0; scan(); }, 120);
  }

  function autoClick() {
    if (!settings.enabled || !settings.autoConfirm || !candidate || !candidateAutoClickable || clicked.has(candidate) || !enabled(candidate) || !buttonMatch(candidate)) return false;
  ...
  function manualClick() {
    ...
    if (confirm(`是否点击按钮：「${label}」？`)) { click(button); toast(`已点击：「${label}」`); requestScan(); }
  }
  ...
  function prepare(el) {
    ...
    if (!raw || pluginPrompt(raw) || raw.includes("[SafeConfirm Supervision]")) return false;
    if (task.promptInjected) return false;
    if (!setInput(el, `${raw.trim()}\n\n${SUPERVISE}`)) return false;
    task.promptInjected = true;
  ...
  function finalBody(raw) {
    ...
    let body = value.slice(start).replace(/^\s*SCH_FINAL\s*/i, "");
    const end = body.search(/<\/\s*SCH_FINAL\s*>/i);
    if (end >= 0) body = body.slice(0, end);
    return body.trim();
  }
  function parseFinal(raw) {
    const body = finalBody(raw);
    if (!body) return null;
    const out = { raw: body };
