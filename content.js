...
现在继续推进原始任务，但不能为了推进而破坏用户已经设定的工程约束。`;
  ...
  function completionIntent(raw) { return /任务全部完成|已经完成全部|全部要求已完成|可以结束|任务已完成|已完成全部|ready_to_stop|ready to stop|task completed|task is complete|all requirements completed/i.test(String(raw || "")) && !blockedOrIncomplete(raw); }
